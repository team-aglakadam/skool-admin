...
export async function GET(req: Request) {
  ...
  const formattedTeachers = data.map((teacher) => {
    ...
    return {
      ...
      employmentType:
        teacher.designation === "full-time"
          ? "full-time"
          : teacher.designation === "part-time"
          ? "part-time"
          : "contract",
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: `Teacher ${userData.full_name} has been created successfully!`,
      },
      { status: 201 }
    );
  } catch (error) {
...
export async function DELETE(req: Request) {
  try {
    ...
    // 1. Get the teacher record to find the linked user_id
    ...
    try {
      ...
      const { error: authDeleteError } =
        await serviceClient.auth.admin.deleteUser(teacher.user_id);

      if (authDeleteError) {
        console.error("Failed to delete auth user:", authDeleteError);
        // You can choose whether to return error or not
      }
    } catch (err) {
